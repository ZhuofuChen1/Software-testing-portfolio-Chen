...
public class DroneCapability {
    ...
    // ---- helpers ----
    public boolean supportsCapacity(double req) {
        return capacity >= req;
    }

    public boolean supportsTemperature(boolean needCooling, boolean needHeating) {
        if (needCooling && !cooling) return false;
        if (needHeating && !heating) return false;
        return true;
    }
}
